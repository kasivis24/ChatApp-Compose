...
class SearchViewModel @Inject constructor(private val database: Database): ViewModel() {
    ...
    private val _searchProfileList = MutableLiveData<List<SearchData>>(emptyList())
    val searchProfileList : LiveData<List<SearchData>> = _searchProfileList
    ...
    fun searchProfile(searchQuery : String,uId : String) {
        ...
        viewModelScope.launch {
            ...
            database.searchProfile(searchQuery,uId).observeForever {
                _searchProfileList.value = it
                Log.d("LogData","ViewModel Data ${_searchProfileList.value}")
            }
